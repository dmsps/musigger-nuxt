...
const getters = {
    IS_GENRES_LOADED: state =>
        !!(state.Genres && typeof state.Genres !== "undefined" && state.Genres.length > 0),
    IS_LABELS_LOADED: state =>
        !!(state.Labels && typeof state.Labels !== "undefined" && state.Labels.length > 0),
}

const actions = {
    [GENRE_LIST_REQUEST]({ commit }) {
