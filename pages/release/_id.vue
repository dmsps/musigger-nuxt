                ...
                <div class="srelease-info auto cell grid-y">
                    ...
                    <p>
                        ...
                        <span v-for="artist in artists" :key="artist">
                            <router-link
                                :to="{ name: 'index', query: { artists: artist } }"
                    ...
                    <div class="grid-x">
                        ...
                        <span class="srelease-info-rating">
                            <v-icon name="star" />
                            {{ Release.Rating }}
                        </span>
                        <span class="srelease-info-votes">
                            <v-icon name="users" />
                            {{ Release.Votes }}
                        </span>
                        <span class="srelease-info-date">
                            <v-icon name="calendar" />
                            {{ Release.Date }}
                        </span>
                        <router-link
                            v-if="Release.Label !== ''"
                            :to="{ name: 'index', query: { labels: Release.Label } }"
                    ...
                    <div class="srelease-extra grid-y">
                        <div class="srelease-extra-info cell" v-html="Release.Info" />
                        <div class="cell grid-x">
                            <div class="srelease-extra-links auto cell" v-html="Release.Links" />
                            <div class="srelease-extra-tofreake small-4 cell">
                                <a
                                    ...
                                    target="_blank"
                                >
                                    <v-icon name="external-link-square-alt" />
                                    Link
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="cell grid-y">
                            <Disqus
                                :shortname="TITLE"
                                :identifier="$route.params.id.toString()"
                                :url="URL"
                            />
                        </div>
                    </div>
                </div>
                <img v-lazy="Release.Cover" class="srelease-cover" :alt="Release.Name" />
            </div>
        </div>
...
import { RELEASE_REQUEST } from "@/utils/actions/release"
...
export default {
    ...
    methods: {
        getSingleRelease(releaseid) {
            const ReleaseData = this.$store.getters["Release/GET_RELEASE_BY_ID"](
            ...
            if (ReleaseData) {
                ...
                this.switchLoading()
                this.$store
                    .dispatch("Release/" + RELEASE_REQUEST, {
                        ID: releaseid,
                    })
                    .then(response => {
    ...
    head() {
        return {
            ...
            link: [
                { rel: "icon", type: "image/x-icon", href: this.FilesUrl + this.Release.MiniCover },
            ],
        }
    },
}
